...
describe("POST /joconde", () => {
	...
	test(`It should create a notice for user ${JSON.stringify(
		jocondeUserOk,
	)}`, async () => {
		const res = await createNotice(await createUser(jocondeUserOk), 200);
		expect(res.success).toBe(true);
	});
	const jocondeUserNotOk = {
		group: "joconde",
		role: "producteur",
		museofile: ["M9999"],
	};
	test(`It should not authorize user ${JSON.stringify(
		jocondeUserNotOk,
	)}`, async () => {
...
describe("PUT /joconde/:ref", () => {
	...
	test(`It should update a notice for ${JSON.stringify(
		jocondeUserOk,
	)}`, async () => {
	...
	test(`It should not authorize ${JSON.stringify(
		jocondeUserOk,
	)} to change MUSEO`, async () => {
		const user = await createUser(jocondeUserOk);
		let res = await createNotice(user, 200);
...
describe("DELETE /joconde/:ref", () => {
	...
	test(`It should not authorize user ${JSON.stringify(
		...
		let res = await createNotice(
			await createUser({ email: "lol@example.com" }),
			200,
		);

...
notices.forEach(function( aRow ){
	...
	//COM2
	var arrayCom2 = (!Array.isArray(notice.COM2)) ? notice.COM2.split(';').map((element) => { return element.trim() !== "" }) : notice.COM2;
 
	//INSEE2
	var arrayInsee2 = (!Array.isArray(notice.INSEE2)) ? notice.INSEE2.split(';').map((element) => { return element.trim() !== "" }) : notice.INSEE2;
 
	//ATEL
	var arrayAtel = (!Array.isArray(notice.ATEL)) ? notice.ATEL.split(';').map((element) => { return element.trim() !== "" }) : notice.ATEL;

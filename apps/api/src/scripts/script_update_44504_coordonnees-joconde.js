notices = db.joconde.find( { $or:[ { $and:[{ "POP_COORDONNEES.lat": { $eq: 0 } }, { MUSEO: { $gt: "" } } ] }, { $and: [{ "POP_COORDONNEES.lon": { $eq: 0 } }, { MUSEO: { $gt: "" } } ] } ] } ).noCursorTimeout();
      ...
      db.joconde.update(
         { REF : notice.REF },
         {
             $set : {
               ...
               POP_CONTIENT_GEOLOCALISATION : noticeMuseo.POP_CONTIENT_GEOLOCALISATION
             }
         }
     )
   }

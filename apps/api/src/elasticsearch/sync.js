...
async function run() {
  ...
  const tasks = new Listr([
    ...
    },
    {
      ...
      task: async () => {
        ...
        await mongoose.connect(mongoUrl, { useNewUrlParser: true, useUnifiedTopology: true, dbName });
        return;
      }
    }
  ]);
  ...
  tasks.add({
    ...
    task: (ctx, task) => {
      return task.newListr(opts.indices.map((db) => ({
        title: db,
        task: async (ctx, task) => {
          ...
          return task.newListr([
            {
              ...
              task: async (ctx, task) => {
                ...
                if ((ovh && !response.body.acknowledged) || (!ovh && !response.acknowledged)) {
                  ctx.error = true;
                ...
                ctx.rootExists = ovh ? existsRes.body : existsRes;
                  ...
                  while (true) {
                    try {
                      ...
                      for (let i in notices) {
                        ...
                        if (ovh) {
                          bulk.push({
                            update: { _index: dbname, _id: notice._id }
                          });
                        } else {
                          bulk.push({
            ...
            },
            {
              ...
              task: async () => {
                ...
                const res = await es.indices.getAlias({ index: `${rootDbname}*` });
                ...
                if (previousDbname) {
                  ...
                  if ((ovh && !response.body.acknowledged) || (!ovh && !response.acknowledged)) {
                    ...
                    throw new Error("Fatal error while creating new alias and removing previous db!");
                  }
                }
              }
            },
            {
              ...
              task: async () => {
                ...
                if ((ovh && !response.body.acknowledged) || (!ovh && !response.acknowledged)) {
                  console.error(response);
                  throw new Error("Fatal error while creating alias!");
                }
              }
            }
          ]);

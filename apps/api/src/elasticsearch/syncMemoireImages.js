...
function checkIfMemoireImageExist(notice) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log("erreur lors de mise à jour image memoire" + e);
    }
  });
}

async function updateNotice(collection, REF, notice) {
              ...
              return new Observable(async observer => {
                ...
                while (true) {
                  ...
                  for (let i in notices) {
                    ...
                    //Dans le cas des notices Palissy et mérimée
                    //On corrige les liens cassés des images mémoire
                    if(db=="palissy"){
                      if (notice.MEMOIRE) {
                        notice.MEMOIRE = await checkIfMemoireImageExist(notice);
                        /* if(notice.MEMOIRE.length>0){
                          notice.CONTIENT_IMAGE = 'oui';
                        }
                        else{
                          notice.CONTIENT_IMAGE = 'non';
                        } */
                        updateNotice(Palissy, notice.REF, notice);
                      }
                    }else if(db=="merimee"){
                      if (notice.MEMOIRE) {
                        notice.MEMOIRE = await checkIfMemoireImageExist(notice);
                        ...
                        updateNotice(Merimee, notice.REF, notice);
                      }
                    }
                    bulk.push({

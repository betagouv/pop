...
const ExceptionMessages = {
	...
	Bad_ARGUMENTS_WITH_TOKEN:
		'Just argument "verb" must be present with "resumptionToken".',
	ID_DOES_NOT_EXIST:
...
function generateException(queryContent, code) {
	...
	if (
		...
		newException["OAI-PMH"].push({
			request: [{ _attr: queryContent }, baseUrl],
		});
	}
	newException["OAI-PMH"].push({
		error: [{ _attr: { code } }, getExceptionMessage(code)],
	});

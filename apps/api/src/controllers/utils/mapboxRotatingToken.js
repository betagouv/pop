...
const getAllTokens = async () => {
	const { body } = await tokenService.listTokens({}).send();
	return body;
};
...
const filterPublicRotatingTokens = (tokens) =>
	tokens.filter(({ note }) => note === PUBLIC_ROTATING_TOKEN_NOTE);
...
const getToken = async () => {
	...
	const [valideTokens, expiredTokens] =
		splitExpiredTokens(publicRotatingTokens);

		...
		if (["producteur", "administrateur"].includes(user.role)) {
			...
			if (group && group.LABEL && group.PRODUCTEURS) {
				...
				group.PRODUCTEURS.map((producteur) => {
					...
					if (
						String(notice.PRODUCTEUR) === String(producteur) ||
						collection === "museo"
					) {
...
async function canUpdateJoconde(user, prevNotice, newNotice) {
	return (
		(await canManageJoconde(user, prevNotice)) &&
		(await canManageJoconde(user, newNotice))
	);
}
...
async function canUpdateAutor(user, prevNotice, newNotice) {
	return (
		(await canManageAutor(user, prevNotice)) &&
		(await canManageAutor(user, newNotice))
	);
}
...
async function canUpdateMnr(user, prevNotice, newNotice) {
	return (
		(await canManageMnr(user, prevNotice)) &&
		(await canManageMnr(user, newNotice))
	);
}
...
async function canUpdateMemoire(user, prevNotice, newNotice) {
	return (
		(await canManageMemoire(user, prevNotice)) &&
		(await canManageMemoire(user, newNotice))
	);
}
...
async function canUpdatePalissy(user, prevNotice, newNotice) {
	return (
		(await canManagePalissy(user, prevNotice)) &&
		(await canManagePalissy(user, newNotice))
	);
}
...
async function canUpdateMerimee(user, prevNotice, newNotice) {
	return (
		(await canManageMerimee(user, prevNotice)) &&
		(await canManageMerimee(user, newNotice))
	);
}
...
async function canUpdateMuseo(user, prevNotice, newNotice) {
	return (
		(await canManageMuseo(user, prevNotice)) &&
		(await canManageMuseo(user, newNotice))
	);
}
...
async function canUpdateEnluminures(user, prevNotice, newNotice) {
	return (
		(await canManageEnluminures(user, prevNotice)) &&
		(await canManageEnluminures(user, newNotice))
	);
}

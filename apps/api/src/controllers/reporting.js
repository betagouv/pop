...
async function recipients(to) {
	switch (to) {
		...
		case "inv":
			return [
				"geraud.buffa@culture.gouv.fr",
				"jean.davoigneau@culture.gouv.fr",
				"ines.graillat@culture.gouv.fr",
			].join(",");
		case "joconde":
			return [
				"sophie.daenens@culture.gouv.fr",
				"angelina.meslem@culture.gouv.fr",
			].join(",");
		case "memoire":
			return [
...
router.post(
    ...
    */
		...
		try {
			...
			return res
				.status(403)
				.send({ success: false, msg: "Impossible d'envoyer le mail" });
		}
	},
);

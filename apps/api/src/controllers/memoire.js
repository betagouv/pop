...
async function withFlags(notice) {
	...
	["NUMTI", "NUMP"]
		.filter(
			(prop) => notice[prop] && !validator.isAlphanumeric(notice[prop]),
		)
	...
	if ("MPP" === notice.PRODUCTEUR) {
		checkRegion =
			...
			String(notice.PAYS[0]).toLowerCase() === "france";
	}
...
async function updateMemoireImageForNotice(
	notice,
	REF,
	IMG = "",
	COPY = "",
	...
	try {
		...
		for (let i = 0; i < linkedNotices.length; i++) {
			if (!LBASE.includes(linkedNotices[i].REF)) {
				promises.push(
					removeMemoireImageForNotice(linkedNotices[i], REF),
				); // Delete  links.
			} else {
    ...
    */
		...
		try {
			...
			for (let i = 0; i < req.files.length; i++) {
				...
				const path = `memoire/${filenamify(notice.REF)}/${filenamify(
					f.originalname,
				)}`;

...
export default class Joconde extends Notice {
	constructor(body) {
		...
		if (body.LOCA) {
			const { LOCA, MANQUANT, MANQUANT_COM } = this.extractManquant(
				body.LOCA,
				body.MANQUANT,
				body.MANQUANT_COM,
			);
	...
	validate(body) {
		...
		["LVID"]
			...
			.forEach((prop) =>
				this._warnings.push(
					`Le champ ${prop} doit être une URL valide`,
				),
			);
	}
	...
	extractManquant = function (loca, manquant, manquantcom) {
		if (
			this.LIST_MANQUANT.some((e) => loca.toLowerCase().indexOf(e) !== -1)
		) {
			const element = this.LIST_MANQUANT.find(
				(e) => loca.toLowerCase().indexOf(e) !== -1,
			);
			return {
				LOCA: this.removeElemenLoca(loca, element),
				MANQUANT: ["manquant"],
				MANQUANT_COM: manquantcom,
			};
		}

		if (this.LIST_VOLE.some((e) => loca.toLowerCase().indexOf(e) !== -1)) {
			const element = this.LIST_VOLE.find(
				(e) => loca.toLowerCase().indexOf(e) !== -1,
			);
			return {
				LOCA: this.removeElemenLoca(loca, element),
		...
		if (
			this.LIST_DISPARU.some((e) => loca.toLowerCase().indexOf(e) !== -1)
		) {
			const element = this.LIST_DISPARU.find(
				(e) => loca.toLowerCase().indexOf(e) !== -1,
			);
		...
		if (this.LIST_PILLE.some((e) => loca.toLowerCase().indexOf(e) !== -1)) {
			const element = this.LIST_PILLE.find(
				(e) => loca.toLowerCase().indexOf(e) !== -1,
			);
		...
		if (
			this.LIST_PRESUME.some((e) => loca.toLowerCase().indexOf(e) !== -1)
		) {
			const element = this.LIST_PRESUME.find(
				(e) => loca.toLowerCase().indexOf(e) !== -1,
			);
			return {
				LOCA: this.removeElemenLoca(loca, element),
				MANQUANT: ["manquant"],
				MANQUANT_COM: "présumé détruit",
			};
		}
...
Joconde.has = (key) => Mapping.joconde[key] !== undefined;

Joconde.convertLongNameToShort = (str) =>
	str

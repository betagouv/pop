...
export default class Palissy extends Notice {
	...
	validate(body) {
		...
		["DOSS", "ETUD", "COPY", "TICO", "CONTACT", "REF"]
			...
			.forEach((prop) =>
				this._warnings.push(`Le champ ${prop} ne doit pas être vide`),
			);
		...
		[
			...
			.filter(
				([existingProp, requiredProp]) =>
					body[existingProp] && !body[requiredProp],
			)
		...
		if (body.DPT && body.DPT.length > 0) {
			const arrayDpt = Array.isArray(body.DPT)
				? body.DPT
				: body.DPT.split(";");
		...
		if (body.INSEE && body.INSEE.length > 0) {
			const arrayInsee = Array.isArray(body.INSEE)
				? body.INSEE
				: body.INSEE.split(";");
			arrayInsee.forEach((val) => {
				...
				if (body.DPT && body.DPT.length > 0) {
					...
					if (
						val &&
						!body.DPT.includes(val.substring(0, 2)) &&
						!body.DPT.includes(val.substring(0, 3))
					) {
						this._errors.push(
							"INSEE et DPT doivent commencer par les deux même lettres",
						);
					}
				}
			});
		...
		if (body.REF && body.REF.length > 10) {
			this._errors.push(
				"La longueur du champ REF ne doit pas dépasser 10 caractères",
			);
		}
		...
		["DOSURL", "DOSURLPDF"]
			...
			.forEach((prop) =>
				this._warnings.push(
					`Le champ ${prop} doit être un lien valide`,
				),
			);
		...
		if (body.REG && body.REG.length > 0) {
			const arrayReg = Array.isArray(body.REG)
				? body.REG
				: body.REG.split(";");
			arrayReg.forEach((val) => {
				if (!regions.includes(val)) {
					this._warnings.push(
						`Le champ REG doit être une région valide : ${regions.join(
							", ",
						)}`,
					);
				}

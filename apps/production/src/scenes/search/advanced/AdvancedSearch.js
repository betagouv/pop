...
const changeValueProducteurMh = (fields, mappingCollection, values) => {
	return fields.map((element) => {
		if (element && !Array.isArray(element.value)) {
			...
			if (
				isMhProducteur(values) &&
				["INSEE", "WEB", "DENQ"].includes(k)
			) {
...
export default function AdvancedSearch({ collection, card }) {
		...
		case "memoire":
			fields = [
				...
				},
				{
					...
					text: "DOM, EDIF, MCL, LEG, SERIE, TITRE, OBJT, DENO, TICO, SUJET - Sujet de la photographie",
				},
				{
				...
				},
				{
					...
					text: "NUMP, ANUMP, NEGPOS, NUMAUTP, NUMOR, ANUMOR, RENV, NUMTI, ANUMTI, REPRO, COTECOR, COTECTI, PRECOR, REF, NUMVERS - Numéro ou cote",
				},
				{
				...
				{
					value: [
						"HISTORIQUE.nom.keyword",
						"HISTORIQUE.prenom.keyword",
						"HISTORIQUE.email.keyword",
						"HISTORIQUE.date.keyword",
						"HISTORIQUE.updateMode.keyword",
					],
	...
	const [sortKey, setSortKey] = useState(
		initialValues.get("sortKey") || "REF.sort",
	);
	const [sortOrder, setSortOrder] = useState(
		initialValues.get("sortOrder") || "desc",
	);
			...
			<Elasticsearch
				...
				onChange={(values) => {
					...
					if ("palissy" === collection) {
						fields = changeValueProducteurMh(
							fields,
							Mapping[collection],
							values,
						);

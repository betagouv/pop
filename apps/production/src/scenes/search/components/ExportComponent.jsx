...
function Loading({ onFinish, collection, ctx, target, header }) {
	useEffect(() => {
		async function exportAndDownload(query) {
			...
			try {
				...
				let rawResponse = await fetch(
					`${es_url}/scroll?index=${collection}`,
					{
						method: "POST",
						headers,
		...
		const query = {
			bool: { must: queries.length === 0 ? { match_all: {} } : queries },
		};
	...
	if (search && header) {
		if (search.qb) {
			...
			const queries = JSON.parse(search.qb).map((s) => {
				const operatorAsText = utils.operators.filter(
					(o) => s.operator === o.value,
				)[0].text;
				const combinatorAsText =
					s.combinator.toLowerCase() === "and" ? "et" : "ou";
				return `${combinatorAsText} ${s.field.replace(
					".keyword",
					"",
				)} ${operatorAsText} ${s.value}`;
			});
			...
			csv.push(
				`"Critères de recherche : ${queryAsText.replace(/"/g, "")}"`,
			);
		} else {

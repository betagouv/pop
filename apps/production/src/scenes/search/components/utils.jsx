...
function customQuery(query, primaryFields, secondaryFields = []) {
	...
	if (query.match(/"[^"]*"| -| \+/)) {
		return {
			simple_query_string: { query, default_operator: "and", fields },
		};
	}
	...
	return {
		bool: { should: [exactRef, ...exactTerm, strict, strictCross, fuzzy] },
	};
}
		...
		return (
			<>
				<Alert color="warning">
					...
					affichées. Cliquez sur « Exporter » pour obtenir la liste
					complète ou affinez votre recherche.
				</Alert>
				{pagination}
			</>
		);

...
class Import extends React.Component {
	...
	renderMuseoFiles() {
		const options = (this.props.museofile || []).map((o) => (
		...
		return (
			...
			>
				...
				<select
					onChange={(e) =>
						this.setState({ museofile: e.target.value })
					}
				...
				<Importer
					...
					dropzoneText={
						<span>
							...
							<b>joconde</b> (<code>.txt</code> ou{" "}
							<code>.csv</code>) et les images associées (au
...
function readme() {
	...
	return (
		<div>
			<h5>Joconde</h5>
			<div>
				Cet onglet permet d’alimenter la base Joconde. Avant tout import
				dans la base, veuillez prendre connaissance de la{" "}
				<a
				...
				relative au versement des données dans la base Joconde <br />{" "}
				<br />
				...
				1) Si présence du champ Adresse d'images (REFIM) alors l'image
				(.jpg) est obligatoire.
				<br />
				2) Sur une notice déjà existante, je peux cliquer sur "Ajouter
				une image" et télécharger une image depuis mon ordinateur. Le
				champ IMG contiendra le lien de l'image ainsi téléchargée.
				<br />
				<br />

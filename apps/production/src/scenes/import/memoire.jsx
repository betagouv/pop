				...
				<Importer
					...
					dropzoneText={
						<span>
							...
							<b>mémoire</b> (extension <code>.ods</code>) et les
							images associées (au format <code>.jpg</code>) dans
							cette zone
						</span>
					}
				/>
	...
	return new Promise((resolve, reject) => {
		...
		utils.readODS(objectFile).then((data) => {
			...
			for (let i = 0; i < data.length; i++) {
				...
				if (!obj.REF) {
					reject(
						`La notice ligne ${i + 1} n'a pas de reference valide`,
					);
					return;
				}
...
function readme() {
	...
	return (
		<div>
			...
			<div>
				Cet onglet permet d’alimenter la base Mémoire pour la partie
				Archives photographiques. <br /> <br />
				...
				1) A l'import, dans mon fichier, je renseigne la notice
				concernée en précisant le champ REF, ainsi que le champ : IMG ou
				REFIMG avec le .jpeg de l'illustration.
				<br />
				<br />
				2) Directement depuis une notice développée : je peux cliquer
				sur "Ajouter une nouvelle image" et télécharger une nouvelle
				image directement depuis mon ordinateur. La notice Mémoire
				reçoit alors dans son champ IMG le contenu .jpeg de l'image
				téléchargée.
				<br />
				Si le champ LBASE contient bien la REF Mérimée ou Palissy MH
				associée, alors l'image ainsi stockée dans IMG pourra également
				illustrer une notice MH associée.
				<br />
				<br />

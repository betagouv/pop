...
export default class ProducteurLine extends React.Component {
	...
	render() {
		const prods = [<option key={""}>{""}</option>];
		prods.push(
			this.props.producteurs.map((e) => (
		...
		return (
			<div className="inline-container">
				<Input
					...
					onChange={(e) =>
						this.setProducteur(this.props.index, e.target.value)
					}
				>
					{prods}
				</Input>
				<Button
					className="deleteProducteurButton"
					onClick={() =>
						this.props.deleteProducteurLine(this.props.index)
					}
				>
					-
				</Button>

...
export const renderImport = (ImportComponent) => {
	...
	return render(<Provider store={store}>{ImportComponent}</Provider>);
};
...
export const summaryNewDocsCount = (count = "[0-9]+") => {
	return new RegExp(
		`.*?(${count}) (?:sont|est) (?:des|une) nouvelles? notice.*`,
	);
};

export const summaryInvalidDocsCount = (count = "[0-9]+") => {
	return new RegExp(
		`.*?(${count}) notice.? ne peu(?:vent|t) être importée.*`,
	);
};

...
import { emailContactMnr, pop_url } from "../../src/config";
...
export default class extends React.Component {
	...
	async componentDidMount() {
		...
		EAnalytics.track([
			"path",
			`Notice Mnr ${this.props.notice?.REF}`,
			"pagegroup",
	...
	render() {
		...
		const { title, images, metaDescription, image_preview } =
		...
		const App = () => (
			<div>
				<PDFDownloadLink
					...
					onClick={() => trackDownload(`mnr_${notice.REF}.pdf`)}
				>
						...
						<Row>
							...
							<Col md="4">
								...
								<div className="sidebar-section info">
									...
									<div>
										<Field
											title={mapping.mnr.REF.label}
											content={notice.REF}
										/>
										<Field
											title={mapping.mnr.BASE.label}
											content={notice.BASE}
										/>
										<Field
											title={mapping.mnr.DMAJ.label}
										...
										/>
										<Field
											title={mapping.mnr.NOTE2.label}
											content={notice.NOTE2}
										/>
									</div>
									<ContactUs
										contact={
											notice.CONTACT || emailContactMnr
										}
										REF={notice.REF}
										base="mnr"
									/>

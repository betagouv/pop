...
export default class extends React.Component {
	...
	static async getInitialProps({ query: { id }, asPath }) {
		...
		const museo =
			notice && notice.MUSEO && (await this.loadMuseo(notice.MUSEO));
	...
	async componentDidMount() {
		...
		EAnalytics.track([
			"path",
			`Notice Joconde ${this.props.notice?.REF}`,
			"pagegroup",
	...
	intersperseReact(arr, separator) {
		...
		Children.forEach(arr, (child, index) => {
			if (isValidElement(child)) {
				...
				result.push(
					createElement("text", { key: `item-${index}` }, child),
				);
			}
	...
	render() {
		...
		const { title, image_preview, metaDescription, images } = getNoticeInfo(
			this.props.notice,
		);
		...
		const obj = {
			name: title,
			created_at:
				notice.PERI != null && notice.PERI.length > 0
					? notice.PERI[0]
					: "",
						...
						<Row>
							<Col md="8">
								<div className="notice-details">
									...
									/>
									{renderFieldRepr(
										mapping.joconde.REPR.label,
										this.linkRepr(this.props.notice.REPR),
									)}
									{/* <Field
							...
							<Col md="4">
								...
								<div className="sidebar-section info">
									...
									<div>
										...
										/>
										<Field
											title={mapping.joconde.REDA.label}
											content={notice.REDA}
											separator="#"
											upper={false}
										/>
										...
										/>
										<Field
											title={mapping.joconde.DATA.label}
											content={notice.DATA}
											separator="#"
											upper={false}
										/>
										<Field
											title={mapping.joconde.LOCA3.label}
											content={notice.LOCA3}
											separator="#"
											upper={false}
										/>
									</div>
		...
		arr.push(
			<Field
				...
				content={
					<Link href={notice.RETIF}>
						<a
							href={`mailto:${notice.MSGCOM}`}
							onClick={handleClick}
						>
							Demande de photographie et/ou de conditions
							d'utilisation
						</a>
					</Link>
				}

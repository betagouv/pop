...
export function getNoticeInfo(notice) {
	...
	switch (base) {
		case "Collections des musées de France (Joconde)": {
			...
			const subtitle =
				!notice.TITR && notice.DENO ? "" : notice.DENO.join(", ");

			const metaDescription = capitalizeFirstLetter(
				jocondeMetaDescription(notice),
			);
			...
			const image_preview = images.length
				? images[0].src
				: "/static/noimage.png";
		...
		case "Photographies (Mémoire)": {
			...
			if (notice.PRODUCTEUR === "CRMH") {
				...
				notice.PRODUCTEUR === "MAP" ||
				notice.PRODUCTEUR === "MPP"
			) {
				logo = "/static/mpp.png";
			...
			const metaDescription = capitalizeFirstLetter(
				memoireMetaDescription(notice),
			);

			const image_preview = notice.IMG
				? `${bucket_url}${notice.IMG}`
				: "/static/noimage.png";
			const images = notice.IMG
				? [{ src: `${bucket_url}${notice.IMG}`, alt: title }]
				: [];
		...
		case "Répertoire des Musées de France (Muséofile)": {
			...
			const image_preview = notice.PHOTO
				...
				: "/static/noimage.png";
			const images = notice.PHOTO
				? [{ src: `${bucket_url}${notice.PHOTO}`, alt: title }]
				: [];
		...
		case "Patrimoine mobilier (Palissy)": {
			...
			if (Array.isArray(notice.DPT) && notice.DPT.length > 0) {
				if (
					...
					for (let i = 0; i < notice.DPT_LETTRE.length; i++) {
						localisation.push(
							`${notice.DPT_LETTRE[i]} (${notice.DPT[i]})`,
						);
					}
				} else {
			...
			if (
				(Array.isArray(notice.WCOM) && notice.WCOM.length > 0) ||
				(Array.isArray(notice.COM) && notice.COM.length > 0)
			) {
...
function memoireMetaDescription(n) {
	const cleanAutor = n.AUTP && String(n.AUTP.join(", ")).trim();
...
export function trackDownload(filename) {
	...
	EAnalytics.pushEvent("download", ["téléchargement"]);
	EAnalytics.pushEvent("globalarg", []);
}

function compare(importedObject, existed) {
  ...
  for (var p in imported) {
    ...
    if (
      imported[p] == existed[p] ||
      (imported[p] === undefined && existed[p] === "") ||
      (imported[p] === undefined &&
        Array.isArray(existed[p]) &&
        !existed[p].length)
    )
      continue;
    ...
    if (Array.isArray(imported[p]) && Array.isArray(existed[p])) {
      if (
        imported[p].length == existed[p].length &&
        imported[p].every((v, i) => v === existed[p][i])
      )
        continue;
    }
    ...
    for (var j = 0; j < existingNotices.length; j++) {
      ...
      if (importedNotices[i].REF.value === existingNotice.REF) {
        ...
        importedNotices[i]._messages = differences.map(e => {
          const from = Array.isArray(existingNotice[e])
            ? existingNotice[e].join(", ")
            : existingNotice[e];
